function x = minReactionForce(d2q,dq,q)
...
% J = -1/2*w'*w;  % Cost function to be maximized
% x = k0*dJ/dq = -k0*w'*dw/dq = -k0*dw'/dq*w;  % Arbitrary joint velocity

k0 = 1e-4;
% dw = -k0*((dtaudq')*tau + 0.01*(dfdq')*f);
x = -k0*(dtaufdq')*tauf;