...
JW = J(q)*W^(1/2);
% % Psuedo inverse of Jacobian (no damped)
% Jp = M(q)^-1*JW'*(JW*M(q)^-1*JW')^-1;
% % Psuedo inverse of Jacobian (damped)
% Jp = (JW'*JW + mu*M(q))\JW'; 
% % Null-space projection
% N = eye(n) - Jp*JW;
% Secondary task gradient
% dq0 = dampShoulder(dq,q);
% dq0 = minWristVelRel(dq,q);
dq0 = minCOM(q);
% S-matrix
S = zeros(n,n);
S(1,1) = 1;
...
% dqW = (JW'*JW + mu*M(q))\(JW'*(dxd+Kp*e) + mu*M(q)'*W^(1/2)*dq1);   % acc. minimization
%% result
dq = W^(1/2)*dqW;
d2q = (dq - dq1)/T;
q = q + dq*T;